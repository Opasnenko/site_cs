...
$pbWarnsBuyout = function_exists('pb_admin_warns_buyout') ? pb_admin_warns_buyout($pdo) : ['enabled' => false, 'price' => 0];
$tpl->set("{warns_buyout}", !empty($pbWarnsBuyout['enabled']) ? 1 : 0);
$tpl->set("{warns_price}", (float) $pbWarnsBuyout['price']);
$tpl->set("{unblock_price}", function_exists('pb_admin_unblock_price') ? (float) pb_admin_unblock_price($pdo) : 100);
$tpl->set("{currency}", function_exists('pb_currency_label') ? pb_currency_label() : '');
$tpl->compile( 'content' );
$tpl->clear();
