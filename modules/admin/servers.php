...
while($row = $STH->fetch()) {
	$rconServersOptions .= '<option value="'.(int)$row->id.'">'.htmlspecialchars((string)$row->name, ENT_QUOTES, 'UTF-8').'</option>';
...
$modes_options = '<option value="0" selected>Без режима</option>';
foreach(pb_smodes_get_all($pdo) as $mode) {
	$modes_options .= '<option value="'.(int)$mode->id.'">'.htmlspecialchars((string)$mode->name, ENT_QUOTES, 'UTF-8').'</option>';
}
...
$tpl->set("{rcon_servers_options}", $rconServersOptions);
$tpl->set("{modes_options}", $modes_options);
$tpl->set("{api_active}", $act);
$tpl->set("{api_active2}", $act2);
$tpl->set("{mon_gap}", $row->mon_gap);
$tpl->set("{mon_key}", htmlspecialchars($row->mon_key, ENT_QUOTES));
$tpl->compile( 'content' );
$tpl->clear();
