...
class AdminsManager {
	...
	public function set_admin_group($pdo, $user_id, $admin_id, $admin_service_id){
		$removed_had_group = false;

		if(!empty($user_id) && function_exists('pb_groups_manager_revoke_service_group')) {
			$removedGroupIds = array();
			$excludeAdminServiceIds = array();

			if($admin_id != 0) {
				$STH = $pdo->prepare("SELECT `admins__services`.`id`, `services`.`users_group` FROM `admins__services`
				...
				foreach($STH->fetchAll() as $svcRow) {
					$excludeAdminServiceIds[] = (int) $svcRow->id;
					if(!empty($svcRow->users_group)) {
						$removedGroupIds[] = (int) $svcRow->users_group;
					}
				}
			} elseif($admin_service_id != 0) {
				$excludeAdminServiceIds[] = (int) $admin_service_id;

				$STH = $pdo->prepare("SELECT `services`.`users_group` FROM `admins__services`
					LEFT JOIN `services` ON `services`.`id` = `admins__services`.`service`
					WHERE `admins__services`.`id`=:admin_service_id LIMIT 1");
				$STH->setFetchMode(PDO::FETCH_OBJ);
			...
			foreach($removedGroupIds as $removedGroupId) {
				/*
				 * Не снимаем доступ к группе, если пользователю она всё ещё
				 * даётся другой активной услугой (не входящей в удаляемый набор).
				 */
				$placeholders = !empty($excludeAdminServiceIds) ? implode(',', array_fill(0, count($excludeAdminServiceIds), '?')) : '0';
				$STH = $pdo->prepare("SELECT `admins__services`.`id` FROM `admins__services`
					LEFT JOIN `admins` ON `admins`.`id` = `admins__services`.`admin_id`
					LEFT JOIN `services` ON `services`.`id` = `admins__services`.`service`
					WHERE `admins`.`user_id`=? AND `services`.`users_group`=? AND `admins__services`.`id` NOT IN ({$placeholders}) LIMIT 1");
				$STH->execute(array_merge(array($user_id, $removedGroupId), $excludeAdminServiceIds));
				$stillHasGroup = (bool) $STH->fetch();

				if(!$stillHasGroup) {
					pb_groups_manager_revoke_service_group($pdo, $user_id, $removedGroupId);
				}
			}
		}
		...
		if($admin_id != 0) {
			...
			$STH->execute(array( ':admin_id' => $admin_id ));
			$removed_had_group = (bool)$STH->fetch();
		...
		/*
		 * Если истёкшая/удаляемая услуга сама держала повышенную группу (previous_group != 0)
		 * и другой активной привилегированной услуги для отката не нашлось — пользователь
		 * раньше оставался без группы вообще. Возвращаем группу по умолчанию сайта.
		 */
		if($removed_had_group && empty($previous_group)) {
			$STH = $pdo->query("SELECT `stand_rights` FROM `config__secondary` LIMIT 1");
			$STH->setFetchMode(PDO::FETCH_OBJ);
			$row = $STH->fetch();
			if(!empty($row->stand_rights)) {
				$previous_group = $row->stand_rights;
			}
		}
